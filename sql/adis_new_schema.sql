...
DROP TABLE IF EXISTS return_labels;
...
CREATE TABLE users (
  ...
  address_id INT NULL,                        -- optional "default address"
...
CREATE TABLE devices (
  ...
  device_fingerprint VARCHAR(128) NULL UNIQUE,
...
CREATE TABLE cart_items (
  ...
  unit_price_snapshot DECIMAL(10,2) NOT NULL, -- snapshot helps price drift
...
CREATE TABLE returns (
  ...
  order_id INT NOT NULL,
  status ENUM('initiated','approved','label_created','in_transit','received','refunded','exchanged','rejected')
         DEFAULT 'initiated',
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE return_labels (
  ...
  label_url TEXT NULL,
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT fk_return_labels_return FOREIGN KEY (return_id) REFERENCES returns(return_id)
);
